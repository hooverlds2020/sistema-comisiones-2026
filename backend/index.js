...
app.put('/api/ordenes/:id', async (req, res) => {
  try {
    const d = req.body;
    const q = `UPDATE ordenes SET tipo_comision=$1, comisionado=$2, rfc=$3, categoria=$4, adscripcion=$5, lugar=$6, motivo=$7, fecha_inicio=$8, fecha_fin=$9, hora_salida=$10, hora_regreso=$11, medio_transporte=$12, vehiculo_marca=$13, vehiculo_modelo=$14, vehiculo_placas=$15, vehiculo_anio=$16, moneda=$17, clave_programatica=$18, cuota_diaria=$19, importe_combustible=$20, importe_pasajes=$21, importe_pasajes_aereos=$22, importe_congresos=$23, importe_viaticos=$24, importe_otros=$25, importe_total=$26, estatus=$27, fecha_elaboracion=$28, informe_actividades=$29, es_fechas_multiples=$30, periodo_texto=$31, dias_salida=$32, dias_regreso=$33, usuario_modificador=$34 WHERE id=$35 RETURNING *`;
    const v = [limpiar(d.tipo_comision), limpiar(d.comisionado), limpiar(d.rfc), limpiar(d.categoria), limpiar(d.adscripcion), d.lugar, limpiar(d.motivo), d.fecha_inicio, d.fecha_fin, limpiar(d.hora_salida), limpiar(d.hora_regreso), d.medio_transporte, limpiar(d.vehiculo_marca), limpiar(d.vehiculo_modelo), limpiar(d.vehiculo_placas), limpiar(d.vehiculo_anio), limpiar(d.moneda) || 'MXN', limpiar(d.clave_programatica), limpiar(d.cuota_diaria), limpiarNumero(d.importe_combustible), limpiarNumero(d.importe_pasajes), limpiarNumero(d.importe_pasajes_aereos), limpiarNumero(d.importe_congresos), limpiarNumero(d.importe_viaticos), limpiarNumero(d.importe_otros), limpiarNumero(d.importe_total), d.estatus || 'En Proceso', d.fecha_elaboracion, limpiar(d.informe_actividades), d.es_fechas_multiples || false, d.periodo_texto || '', d.dias_salida || '', d.dias_regreso || '', limpiar(d.usuario_modificador), req.params.id];
