...
const registrarBitacora = async (usuario, accion, folio, detalles) => {
  try {
    ...
    console.error('❌ Error guardando en bitácora:', err.message);
  }
};
...
// ��� ELIMINAR ORDEN (BITÁCORA INTEGRADA)
app.delete('/api/ordenes/:id', async (req, res) => {
    try {
        ...
        if (result.rows.length > 0) {
            ...
            registrarBitacora(usuario_modificador, 'ELIMINAR', folioStr, `Se eliminó la orden de ${ordenEliminada.comisionado}`);
            enviarAlertaTelegram(`���️ *ORDEN ELIMINADA*\n\n��� *Folio:* ${folioStr}\n��� *Viajero:* ${ordenEliminada.comisionado}\n���‍��� *Usuario:* ${usuario_modificador || 'Sistema'}`);
...
// ��� CREAR ORDEN (BITÁCORA INTEGRADA)
app.post('/api/ordenes', async (req, res) => {
  try {
    const data = req.body;

    // ���️ BLOQUE DE SEGURIDAD PARA DATOS CORRECTOS
    if (!data.comisionado || data.comisionado.trim() === "" || !data.lugar || data.lugar.trim() === "") {
        console.log("❌ Intento de guardado de orden incompleta bloqueado.");
        return res.status(400).json({ error: "Faltan datos críticos: Comisionado o Lugar" });
    }
...
// ��� ACTUALIZAR ORDEN (BITÁCORA INTEGRADA)
