...
import { Plus, Edit, Trash2, Search, X, Save, ChevronLeft, ChevronRight, Wallet, AlertTriangle } from 'lucide-react';

const ClavesPresupuestalesTable = () => {
  ...
  const cargarClaves = () => {
    fetch('/api/claves-presupuestales')
      .then(res => res.json())
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = form.id ? `/api/claves-presupuestales/${form.id}` : '/api/claves-presupuestales';
    ...
    try {
      ...
      if (response.ok) {
        alert('✅ ¡Clave presupuestal guardada correctamente!');
  ...
  const ejecutarEliminacion = async () => {
    if (claveAEliminar) {
      try {
        await fetch(`/api/claves-presupuestales/${claveAEliminar.id}`, { method: 'DELETE' });
  ...
  const filtrados = claves.filter(c => 
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 bg-white p-6 rounded-xl shadow-sm border border-blue-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black text-blue-900 tracking-tight">Claves Presupuestales</h1>
              <p className="text-gray-500 font-bold text-sm">Catálogo de asignación de presupuesto</p>
            </div>
          </div>
...
export default ClavesPresupuestalesTable;
