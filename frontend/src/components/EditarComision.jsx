...
const EditarComision = () => {
    ...
    fetch(`/api/ordenes/${id}`)
      ...
      .then(data => {
        ...
        if(data.fecha_elaboracion) {
            data.fecha_elaboracion = data.fecha_elaboracion.split('T')[0];
        } else {
            data.fecha_elaboracion = new Date().toISOString().split('T')[0];
        }
        
        const cleanData = { ...data };
        ['importe_combustible', 'importe_otros', 'importe_pasajes_aereos', 'importe_pasajes', 'importe_congresos', 'importe_viaticos', 'cuota_diaria'].forEach(field => {
        ...
        // RECUPERAR UI TRANSPORTE
        if (cleanData.medio_transporte === 'Aéreo') {
            setUiTransporte('Aéreo');
        } else {
            if (cleanData.vehiculo_placas && cleanData.vehiculo_placas.trim().length > 0) {
        ...
        if (cleanData.clave_programatica) {
            if (cleanData.clave_programatica.includes(',')) {
                setClavesSeleccionadas(cleanData.clave_programatica.split(', '));
            } else {
      ...
      .catch(err => {
        ...
        Swal.fire('Error', 'No se pudo cargar la comisión', 'error');
      });
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        Swal.fire({ title: '¡Actualizado!', text: 'La comisión se ha modificado correctamente.', icon: 'success', confirmButtonColor: '#1e3a8a' }).then(() => navigate('/'));
      } else {
        const errorData = await response.json();
        Swal.fire('Error', errorData.error || 'No se pudo actualizar.', 'error');
      }
    } catch (error) { console.error('Error de red:', error); Swal.fire('Error', 'Fallo de conexión con el servidor', 'error'); }
  };
       ...
       <div className="max-w-6xl mx-auto bg-white p-8 rounded-lg shadow border border-gray-200">
         ...
         <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        {uiTransporte === 'Vehículo' ? (
                            <div className="grid grid-cols-3 gap-2 animate-fadeIn bg-white p-3 rounded border border-gray-200">
                                <div><label className="block text-xs font-bold text-gray-700 mb-1">Marca</label><input name="vehiculo_marca" value={formData.vehiculo_marca} onChange={handleChange} className="w-full p-2 border rounded" /></div>
                                <div><label className="block text-xs font-bold text-gray-700 mb-1">Modelo</label><input name="vehiculo_modelo" value={formData.vehiculo_modelo} onChange={handleChange} className="w-full p-2 border rounded" /></div>
                                <div><label className="block text-xs font-bold text-gray-700 mb-1">Placas</label><input name="vehiculo_placas" value={formData.vehiculo_placas} onChange={handleChange} className="w-full p-2 border rounded" /></div>
                            </div>
                        ) : (
            ...
            <div className="flex justify-end gap-4 mt-8 pt-4 border-t">
              ...
              <button type="submit" className="px-8 py-3 bg-blue-900 text-white rounded hover:bg-blue-800 flex items-center gap-2 font-bold shadow-lg transition-transform active:scale-95"><Save size={20}/> Actualizar Comisión</button>
            </div>
         </form>
       </div>
    </div>
