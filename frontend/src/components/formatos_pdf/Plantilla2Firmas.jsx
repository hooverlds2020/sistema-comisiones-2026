...
const Firma = ({ nombre, cargo }) => (
  <View style={{ alignItems: 'center', paddingBottom: 6, justifyContent: 'flex-end', width: '100%', height: '100%' }}>
    <View style={{ borderTopWidth: 1, borderColor: '#000', width: '85%', marginBottom: 2 }} />
    <Text style={{ fontSize: 7.2, fontWeight: 'bold', textAlign: 'center' }}>{nombre}</Text>
    <Text style={{ fontSize: 6.2, textAlign: 'center' }}>{cargo}</Text>
  </View>
);

const FirmaHeader = ({ left, right }) => (
  <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderTopWidth: 1, borderColor: '#000', backgroundColor: '#f0f0f0', minHeight: 18, alignItems: 'center' }}>
    <View style={{ width: right ? '50%' : '100%', paddingVertical: 4, borderRightWidth: right ? 1 : 0, borderColor: '#000' }}>
      <Text style={{ fontSize: 7.5, fontWeight: 'bold', textAlign: 'center' }}>{left}</Text>
    </View>
    {right && (
      <View style={{ width: '50%', paddingVertical: 4 }}>
        <Text style={{ fontSize: 7.5, fontWeight: 'bold', textAlign: 'center' }}>{right}</Text>
      </View>
    )}
  </View>
);
...
const Plantilla2Firmas = ({ data, autoridades = [] }) => {
  if (!data) return null;
  ...
  const autoridadAutoriza = esDirectorViajando ? getAutoridad("SECRETARIO") : getAutoridad("DIRECTOR");
  ...
  const esSaturado = data.es_fechas_multiples && pesoTotalDocumento >= 9;
  const esMuySaturado = data.es_fechas_multiples && pesoTotalDocumento >= 14;
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        <View style={styles.mainTable}>
          ...
          <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderColor: '#000', flex: 1.5, minHeight: 35, alignItems: 'stretch' }}>
            <View style={{ width: '100%', padding: 2, display: 'flex', flexDirection: 'column' }}>
              <View style={{ width: '50%' }}>
                ...
                {parseFloat(data.importe_viaticos) > 0 && <View style={styles.gastosRow}><View style={styles.colGastoDesc}><Text>37511.- VIÁTICOS NACIONALES</Text></View><View style={styles.colGastoMonto}><Text style={{ fontWeight: 'bold' }}>{money(data.importe_viaticos, data.moneda)}</Text></View></View>}
          ...
          <View style={{ flexShrink: 0 }}>
            ...
            <FilaDosFirmas izq={{ nombre: comisionadoNombre, cargo: categoriaComisionado }} der={autoridadAutoriza} altura={firmaH} />
              ...
              <View style={{ alignItems: 'center', paddingTop: 10, paddingBottom: 0 }}>
                <View style={{ alignItems: 'center' }}>
                  ...
                  <View style={{ width: 230, alignItems: 'center' }}>
                    ...
                    <Text style={{ fontSize: 7.2, fontWeight: 'bold' }}>{comisionadoNombre}</Text>
                    <Text style={{ fontSize: 6.2 }}>{categoriaComisionado}</Text>
                  </View>
                </View>
              </View>
            </View>
...
export default Plantilla2Firmas;
